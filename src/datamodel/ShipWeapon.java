...
public class ShipWeapon {
    ...
    /**
     * Data mines a COBJ record to extract all relevant Ship Weapon Stats.  Should only be called
     * on COBJ records that are known to be Ship Weapons
     *
     * @param cobj The {@link COBJRecord} to data mine for the Ship Weapon Stats
     */
    public ShipWeapon(COBJRecord cobj) {
        Assert.assertTrue(KYWDRecord.CATEGORY_SHIPMOD_WEAPON_FID.equals(cobj.getCategoryFormId()),
        ...
        cobj.getConditions().forEachRemaining(c -> {
            // Skip vendor availability conditions
            if (c.isMinLevelCondition()) {
                playerLevel.set(Util.asInt(c.getComparisonValue()));
        ...
        cobj.getRequiredPerks().forEachRemaining(rp -> {
        ...
        GBFMRecord gbfm = Assert.assertNotNull(cobj.getCreatedObject(GBFMRecord.class),
                "Missing Created Object in " + cobj);
        this.make = Assert.assertNotNull(gbfm.getManufacturer(), "No weapon make found in " + gbfm);
        this.weaponClass = Assert.assertNotNull(gbfm.getShipModuleClass(), "No weapon class found in " + gbfm);

        // Data mine the PropertySheet to get hull, mass, crew, health and max power
        GBFMRecord.PropertySheet props = Assert.assertNotNull(gbfm.getPropertySheet(), "Missing Property Sheet in " + gbfm);
        this.hull = props.getPropertyValueAsInt(AVIFRecord.HEALTH_FID, "Missing hull data in " + gbfm);
        this.mass = props.getPropertyValueAsInt(AVIFRecord.SPACESHIP_PART_MASS_FID, "Missing mass data in " + gbfm);
        this.crew = props.getPropertyValueAsDouble(AVIFRecord.SPACESHIP_CREW_RATING_FID, "Missing crew data in " + gbfm, 0.25);
        this.health = props.getPropertyValueAsInt(AVIFRecord.SHIP_SYSTEM_WEAPON_HEALTH_FID, "Missing health data in " + gbfm);
        this.maxPower = props.getPropertyValueAsInt(AVIFRecord.SPACESHIP_WEAPON_POWER_FID, "Missing max power data in " + gbfm);

        // Data mine the WEAP record to get everything else but projectile speed
        WEAPRecord weap = Assert.assertNotNull(gbfm.getWEAPRecord(), "Could not find WEAP record in " + gbfm);
        ...
        AMMORecord ammo = Assert.assertNotNull(weap.getAMMORecord(), "No AMMO record found for " + weap);
        PROJRecord proj = Assert.assertNotNull(ammo.getPROJRecord(), "No PROJ record found for " + ammo);
    ...
    public static void emitHeaders(PrintStream output) {
        output.println("Make|Model|Class|Category|Cost|Hull|Mass|Health|Crew Capacity|Max Power|" +
    ...
    public void emitAsCSV(PrintStream output) {
        output.printf("%s|%s|%s|%s|%d|%d|%d|%d|%f|%d|%d|%d|%s|%d|%d|%d|%f|%d|%d|%f|%f|%f|%d|%s\n",
                make, model, weaponClass, category, cost, hull, mass, health, crew, maxPower, minLevel,
                minStarshipEng, requiredPerk, hullDmg, shieldDmg, emDmg, critBonus, range, speed, rof, rechargeDelay,
                rechargeTime, magazine, turret ? "X" : "");
    }
}
