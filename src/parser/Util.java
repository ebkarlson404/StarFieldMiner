 ...
 */
public class Util {
    private static final Pattern DECORATED_FORMID_REGEX = Pattern.compile(".*\\[[^:]*:([0-9A-F]*)\\].*");

    /**
     * Extracts a raw Form ID from a <i>Decorated Form ID</i>.  <i>Decorated Form IDs</i>
     * have the following syntax: {@code <EditorID>[<Signature>:<RawFormID>]}
     *
    ...
    public static Integer asInt(@NotNull JsonNode value) {
        Assert.assertTrue(value.isNumber() || value.isTextual(), "asInt: value must be a number or a string");
    ...
    public static Double asDouble(@NotNull JsonNode value) {
        Assert.assertTrue(value.isNumber() || value.isTextual(), "asDouble: value must be a number or a string");
